...
# 3. 테스트 데이터 생성 (선택 - 서버 기동 시 자동으로도 실행된다)
python seed.py
...
- 시드는 **서버 기동 시 자동으로 한 번 실행**된다(`main.py` → `seed.seed()`). 이미 있는 데이터는
  건드리지 않으므로 여러 번 켜도 중복되지 않는다. 끄고 싶으면 환경 변수 `SEED_ON_STARTUP=0`
- 테스트 계정: **`testuser` / `test1234`**

## 배포 (Render)

프론트엔드는 Vercel, 백엔드는 Render에 올라가 있다.

### CORS

로컬 개발 주소(`localhost` · `127.0.0.1`의 3000 · 5173)와 **`*.vercel.app`** 은 기본으로 허용된다.
Vercel은 미리보기 배포마다 주소가 달라져서 정규식으로 열어뒀다.

다른 도메인을 쓰면 코드를 고치지 않고 환경 변수로 추가한다.

```
ALLOWED_ORIGINS=https://example.com,https://another.com
```

### ⚠️ 무료 플랜은 디스크가 유지되지 않는다

Render 무료 플랜은 일정 시간 요청이 없으면 인스턴스가 잠들고, 다시 깨어날 때 파일 시스템이
초기화된다. `everytime.db`가 빈 파일로 다시 만들어지므로 **그때까지 쌓인 계정·게시글이 모두 사라진다.**

그래서 기동 시 시드를 자동 실행한다. 덕분에 서버가 깨어난 직후에도 게시판 3개 · 테스트 계정 ·
샘플 게시글이 항상 있어서, 저장된 로그인 정보가 없는 환경(시크릿 모드 등)에서도 바로 로그인해
화면을 확인할 수 있다.

다만 **사용자가 직접 쓴 글은 여전히 사라진다.** 데이터를 계속 보존해야 한다면
SQLite 대신 Render의 관리형 PostgreSQL을 붙이고 `database.py`의 접속 문자열을 바꿔야 한다.

> 잠든 인스턴스를 깨우는 첫 요청은 수십 초 걸릴 수 있다. 발표·시연 전에 한 번 열어두면 좋다.
